...
export default function EditAboutStory() {
    ...
    async function loadContent() {
      try {
        ...
        if (data.content?.story) {
          setStory({ ...EMPTY_STORY, ...data.content.story });
        }
      } catch (err) {
  ...
  const handleAltChange = (alt) => {
    setStory((prev) => ({ ...prev, image: { ...prev.image, alt } }));
  };
  ...
  const handleResetImage = () => {
    setStory((prev) => ({ ...prev, image: { ...DEFAULT_IMAGE } }));
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage({ type: "success", text: "Story section updated successfully." });
    } catch (err) {
      setMessage({ type: "error", text: err.message });
    } finally {
      setSaving(false);
    ...
    <section className="relative px-6 py-10 lg:px-10">
      <div className="mb-8">
        <h1 className="text-2xl font-bold leading-tight text-brand-dark md:text-3xl">
          Edit <span className="text-brand-green">About — Story Section</span>
        </h1>
        ...
        <div>
          ...
          <div className="max-w-xs rounded-lg border border-dashed border-brand-border p-3">
            ...
            <label className="flex cursor-pointer items-center justify-center gap-1.5 rounded-md border border-brand-border px-3 py-1.5 text-xs font-medium text-brand-dark transition-colors hover:border-brand-green hover:text-brand-green">
              <Upload size={14} />
              {story.image?.url ? "Replace" : "Upload"}
              <input
                type="file"
                accept="image/*"
                className="hidden"
